...
def add_detector_to_user(_):
    ...
    new_detector = create_detector_for_mongo(
        detector_id, location_id, detector_name, char_num, coma_position, type
    )
    ...
    mongo.locations.find_one_and_update(
        {"_id": ObjectId(location_id)}, {"$push": {"detectors": new_detector}}
    )
